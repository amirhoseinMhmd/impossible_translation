...
# Load the finetune module from the fine-tuning directory
finetune_path = Path(__file__).parent.parent / "fine-tuning" / "finetune.py"
...
def test_load_configs(temp_files):
    ...
    config = finetune.load_configs(config_file)
    assert config == TEST_CONFIG

def test_load_sentences_from_file(temp_files):
    input_file, _ = temp_files
    sentences = finetune.load_sentences_from_file(input_file)
...
def test_prepare_dataset(mock_tokenizer_from_pretrained):
    ...
    # Use a smaller max_length for easier debugging
    train_dataset, eval_dataset = finetune.prepare_dataset(
    ...
    # Let's trace the logic to find the correct position
    # prompt_length = len(prompt_only_ids) = 6
    # The code finds corrected_marker_tokens at index 4 in full_text_ids.
    # position = 4 + len(corrected_marker_tokens) = 4 + 2 = 6
    # So labels should be [-100, -100, -100, -100, -100, -100, 60, 70, 80, 50256, -100, ...]
...
@patch('finetune.GPT2LMHeadModel.from_pretrained')
@patch('finetune.GPT2Tokenizer.from_pretrained')
...
def test_train_model(mock_trainer, mock_tokenizer, mock_model):
    ...
    finetune.train_model(train_dataset, eval_dataset, TEST_CONFIG, "gpt2", "./test-output")
...
@patch('finetune.train_model')
@patch('finetune.prepare_dataset')
@patch('finetune.generate_training_data')
@patch('finetune.GPT2Tokenizer.from_pretrained')
def test_main(mock_tokenizer, mock_generate_training_data, mock_prepare_dataset, mock_train_model, temp_files):
    ...
    with patch('builtins.open', MagicMock()) as mock_open:
        with patch('json.load', MagicMock(return_value=[("corrupted", "original")])) as mock_json_load:
            finetune.main(TEST_CONFIG, str(input_file), "gpt2", "partialReverse")
